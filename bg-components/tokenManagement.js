			...
			if (content.url.includes("docs.google.com")) {
				if (api && orgId) {
					...
					if (docUuid) {
						...
						await Log("Fetching Google Drive document from attached sync source");
						try {
							const syncText = await api.getSyncText(syncObj);
							if (syncText) {
								textPieces.push(syncText);
								await Log("Retrieved Google Drive document:", syncText.length, "chars");
							}
						} catch (error) {
...
class TokenCounter {
	...
	async getNonTextFileTokens(fileContent, mediaType, fileMetadata, orgId) {
		...
		if (cachedValue !== undefined) {
			await Log("Using cached token count for file", { file_uuid: fileMetadata.file_uuid, cachedValue });
			return cachedValue;
		}
