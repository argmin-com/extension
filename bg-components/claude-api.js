...
class ClaudeAPI {
	...
	async getStyleTokens(styleId, tabId) {
		...
		await Log("Got style:", style?.key || style?.uuid, "prompt length:", style?.prompt?.length || 0);
		if (style) {
...
class MessageAPI {
	...
	async getUploadedFileAsBase64(url) {
		try {
			await Log("Starting file download from Claude attachment URL");
	...
	async getSyncText(sync) {
		...
		await Log("Processing sync:", { syncType, syncId: sync.uuid || sync.id });
		...
		if (syncType === "gdrive") {
			...
			try {
				...
				if (!owner || !repo || !branch || !filters?.filters) {
					await Log("warn", "Incomplete GitHub sync config");
					return "";
				}
				...
				for (const [filePath, action] of Object.entries(filters.filters)) {
					...
					try {
						...
						if (response.ok) {
							...
							await Log("warn", "Failed to fetch GitHub file", { status: response.status });
						}
					} catch (error) {
						await Log("error", "Error fetching GitHub file", error);
					}
				}
				return allContent;
			} catch (error) {
	...
	async getSyncTokens() {
		...
		const syncPromises = (this.data.sync_sources || []).map(async (sync) => {
			await Log("Processing sync source:", { syncType: sync.type, syncId: sync.uuid });
			...
			if (cachedData &&
				...
				if (timeDiff < SYNC_CACHE_TIME_THRESHOLD) {
					await Log("Using cached sync data for:", { syncType: sync.type, syncId: sync.uuid });
					return cachedData.tokenCount;
				}
			}

			// Cache miss - fetch and count
			await Log("Cache miss for sync source:", { syncType: sync.type, syncId: sync.uuid });
...
class ConversationAPI {
		...
		if (!cacheBoundary) {
			...
			if (messageAge >= cache_lifetime) {
				...
				if (currentTrunkIds.has(cacheBoundary.uuid)) {
					...
					await Log("Cache boundary in current trunk - cache available");
				} else {
					...
					while (currentId && currentId !== rootId) {
						if (currentTrunkIds.has(currentId)) {
							...
							await Log("Cache ends at common ancestor");
							break;
						}
						const rawMessage = messageMap.get(currentId);
						currentId = rawMessage?.parent_message_uuid;
	...
	async getInfo(isNewMessage) {
		await Log("API: Requesting information for conversation");
		const conversationData = await this.getData(true);
		...
		for (let i = 0; i < currentTrunk.length; i++) {
			...
			if (message.uuid === cacheEndId) {
				cacheIsActive = false;
				await Log("Hit cache boundary at message");
			}
		}
		...
		await Log("Total tokens for conversation", { lengthTokens, model: conversationModelType });
