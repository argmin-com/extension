...
class SettingsCard extends FloatingCard {
	...
	async build() {
		...
		label.textContent = 'Anthropic API Key (optional):';

		const disclosure = document.createElement('div');
		disclosure.className = 'text-sm';
		disclosure.style.cssText = 'opacity: 0.7; margin-bottom: 8px; line-height: 1.4;';
		disclosure.textContent =
			'When set, some text may be sent to api.anthropic.com for accurate token counting. ' +
			'This can include message text, uploaded file content, Claude profile/style/memory text, ' +
			'and attached Google Drive or GitHub sync text. Without a key, all counting is done locally. ' +
			'The key is stored locally in browser.storage.local.';
		...
		saveButton.addEventListener('click', async () => {
			// HIGH-2: Explicit consent before enabling Anthropic API data transmission
			if (input.value && input.value.length > 0) {
				const consent = confirm(
					'By saving an API key, you authorize this extension to send some text to api.anthropic.com ' +
					'for token counting.\n\n' +
		...
		toggleLabel.textContent = 'Usage reset notifications';
