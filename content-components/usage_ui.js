/* global CONFIG, Log, ProgressBar, sendBackgroundMessage,
   ...
   RED_WARNING, BLUE_HIGHLIGHT, SUCCESS_GREEN, SELECTORS, escapeHtml, replaceInnerHtml, fmtEnergy, fmtCarbon */
'use strict';
...
class UsageSection {
	...
	render(usageData, forecasts = [], sessionStats = null, velocity = null) {
		...
		// Build lookup of forecasts by limit name
		const forecastMap = {};
		for (const fc of forecasts) forecastMap[fc.limitName] = fc;
		...
		for (const limit of activeLimits) {
			...
			replaceInnerHtml(resetTime, this.formatResetTime(limit.resetsAt));

			// Show exhaustion forecast if available
			const fc = forecastMap[limit.key];
			if (fc && fc.exhaustionTimeFormatted) {
				let fcEl = barElements.row.querySelector('.ut-forecast-eta');
				if (!fcEl) {
					fcEl = document.createElement('div');
					fcEl.className = 'text-text-400 text-xs ut-forecast-eta';
	...
	renderResetTimes(usageData) {
		...
		for (const limit of usageData.getActiveLimits()) {
			...
			if (barElements) {
				replaceInnerHtml(barElements.resetTime, this.formatResetTime(limit.resetsAt));
			}
		}
	}
...
class UsageUI {
	...
	createHeader() {
		...
		replaceInnerHtml(settingsButton, `
	...
	renderChatArea() {
		...
		if (sessionMaxed && usageData.hasExtraUsage()) {
			...
			replaceInnerHtml(resetDisplay, getResetTimeHTML(resetInfo));
			return;
		}
	...
	renderResetTimes() {
		...
		// Chat area
		const resetInfo = usageData.getSessionResetInfo();
		replaceInnerHtml(this.elements.chat.resetDisplay, getResetTimeHTML(resetInfo));
	}
	...
	handleUsageUpdate(usageDataJSON) {
		...
		// Fetch forecasts for Claude using the fresh usage data
		sendBackgroundMessage({
			type: 'getForecast',
			platform: 'claude',
			claudeUsageData: usageDataJSON
		}).then(forecasts => {
