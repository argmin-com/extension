...
class LengthUI {
	...
	renderTitleContainer() {
		...
		elements.forEach((element, index) => {
			...
			if (index < elements.length - 1) {
				...
				replaceInnerHtml(sep, separator);
	...
	renderCachedTime() {
		...
		if (diff <= 0) {
			this.state.cachedUntilTimestamp = null;
			replaceInnerHtml(this.elements.titleArea.cached, '');
			this.renderTitleContainer();
	...
	handleConversationUpdate(conversationDataJSON) {
		...
		if (conversationDataJSON.conversationId && currentConvoId &&
			...
			Log('LengthUI: Ignoring stale conversation update');
			return;
		}
